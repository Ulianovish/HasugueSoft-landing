...
import { ArrowLeft, Bot, Zap, Users, Globe, Smartphone, Building2 } from "lucide-react"
...
const questions = [
  {
    ...
    options: ["Startup", "Pequeña Empresa", "Empresa Grande", "Organización Sin Fines de Lucro"],
  },
  {
          ...
          <div className="grid grid-cols-2 gap-4">
            {questions[currentQuestion].options.map((option) => (
              <button
                ...
                className="p-4 border border-gray-700 rounded-lg hover:border-purple-500 hover:bg-purple-500/10 transition-all duration-200 text-left"
              >
                <span className="text-purple-400 font-medium">{option}</span>
              </button>
            ))}
          </div>
    ...
    return (
      <div className="h-full rounded-lg border border-border bg-black text-white p-6 flex flex-col justify-center items-center">
        <div className="text-center space-y-4">
          ...
          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="w-8 h-8 bg-purple-500 rounded-full" />
          </div>
          <h3 className="text-2xl font-bold">¡Gracias!</h3>
          <p className="text-gray-400">
            Revisaremos tus respuestas y te contactaremos con una consulta personalizada.
          </p>
          <button
            onClick={() => {
              setSelectedService(null)
            ...
            className="mt-4 px-6 py-2 bg-purple-500 text-black rounded-lg hover:bg-purple-400 transition-colors"
          >
