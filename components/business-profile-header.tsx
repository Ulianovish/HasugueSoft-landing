...
export function BusinessProfileHeader() {
  ...
  const getDisplayEntries = () => {
    const entries = []
    if (profileData.service) entries.push({ label: "Service", value: profileData.service, color: "text-purple-400" })
    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          className="fixed top-16 left-0 right-0 z-40 bg-black/20 backdrop-blur-2xl border-b border-purple-500/20 shadow-xl"
        >
          {/* Subtle animated background gradient */}
          <div className="absolute inset-0 bg-gradient-to-r from-purple-500/5 via-blue-500/5 to-violet-500/5">
          <div
            className="absolute inset-0 bg-gradient-to-r from-purple-500/3 to-blue-500/3"
              animate={{
                background: [
                    ...
                    >
                      ...
                      {completionPercentage >= 100 && (
                        <motion.div
                          ...
                          className="absolute -top-1 -right-1 w-5 h-5 bg-purple-500 rounded-full flex items-center justify-center shadow-lg"
                        >
                      ...
                      <div className="flex flex-wrap items-center gap-1.5 mb-1.5">
                        {displayEntries.slice(0, 4).map((entry, index) => (
                          <motion.div
                            ...
                            className="flex items-center space-x-1.5 px-2.5 py-1 bg-gray-800/30 backdrop-blur-sm rounded-full border border-gray-700/30 hover:border-purple-500/20 transition-all duration-300"
                          >
                        ...
                        {displayEntries.length > 4 && (
                          <motion.div
                            ...
                            className="px-2.5 py-1 bg-gradient-to-r from-purple-500/15 to-blue-500/15 backdrop-blur-sm rounded-full border border-purple-500/20"
                          >
                            <span className="text-xs text-purple-400 font-semibold">+{displayEntries.length - 4}</span>
                          </motion.div>
                        )}
                      </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                        {displayEntries.map((entry, index) => (
                          <motion.div
                            ...
                            className="bg-gray-800/15 backdrop-blur-sm rounded-lg p-3 border border-gray-700/20 hover:border-purple-500/20 transition-all duration-300"
                          >
